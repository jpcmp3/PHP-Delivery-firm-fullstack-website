...
<div class="container my-5">
  <h1 class="text-center my-3">Списък на пратки</h1>
  <hr>
  
  <a href="add_shipment.php" class="btn btn-dark text-light my-1" style="text-decoration: none">Добави пратка</a>
      ...
      if ($result) {
        while ($row = mysqli_fetch_assoc($result)) {
          ...
          $update_time = $row['update_time'];
                  list($badgeColor, $textColor) = getStatusBadgeColor($status); // Get both badge color and text color
          ...
          echo '<tr>
                  ...
                  <td>' . $tracking_id . '</td>
                  <td>' . $location . '</td>
<td><h5 class="badge" style="background-color:' . $badgeColor . '; color:' . $textColor . ';">' . $status . '</h5>
                  </td>                  <td>' . $update_time . '</td>
                  <td>
                  ...
                  <button type="button" class="btn btn-danger delete-button" data-id="' . $shipment_id . '"><i class="fas fa-trash-alt"></i></button>
                </td>
                </tr>';
        }
      }
